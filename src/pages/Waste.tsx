...
export default function Waste() {
  ...
  return (
      ...
      <div className="flex gap-1 p-2 min-w-max">
        {sections.map(s => (
          <button key={s.id} onClick={() => setActiveSection(s.id)}
            ...
            {'badge' in s && (s as {badge?: number}).badge ? <span className={cn('text-xs px-1 rounded-full font-medium', activeSection===s.id?'bg-white/20 text-white':'bg-primary/15 text-primary')}>{(s as {badge: number}).badge}</span> : null}
          </button>
        ))}
      </div>
    </div>
    ...
    <div className="rounded-xl border border-border overflow-hidden bg-card flex" style={{ minHeight: '580px' }}>
      ...
      {/* Sidebar */}
      <div className="hidden md:flex md:flex-col w-64 shrink-0 border-l border-border bg-muted/30">
        {/* Quick Actions */}
