...
export default function Profits() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Mobile top nav */}
      <div className="md:hidden rounded-xl border border-border bg-card overflow-hidden mb-0">
            ...
            {(['product','customer'] as const).map(g=>(
              <button key={g} onClick={()=>setGroupBy(g)}
                className={cn('px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors',
                  groupBy===g?'bg-primary text-primary-foreground border-primary':'border-border text-muted-foreground hover:bg-muted')}>
