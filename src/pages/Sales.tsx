...
export default function Sales() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Mobile top nav */}
      <div className="md:hidden overflow-x-auto rounded-xl border border-border bg-muted/30 mb-0">
        <div className="flex items-center gap-1 p-2 min-w-max">
          {sections.map(s=>(
            <button key={s.id} onClick={()=>setActiveSection(s.id)}
              ...
              {s.count>0&&<span className={cn('text-xs px-1 rounded-full font-medium',activeSection===s.id?'bg-white/20 text-white':'bg-primary/15 text-primary')}>{s.count}</span>}
            </button>
          ))}
          {canAdd&&<button onClick={()=>{setEditGroup(null);setDrawerOpen(true)}} className="flex items-center gap-1 px-3 py-2 rounded-lg text-xs font-medium bg-primary text-primary-foreground whitespace-nowrap mr-2"><Plus className="w-3.5 h-3.5"/>إضافة</button>}
        </div>
      </div>
